...
OUT = "$HOME/workspace/claudecode/automath/engine/harvest/k1695_r6_r2split_odd/p%d_chart%d" % (p, chart_idx)
...
def run_ideal(tag, assignment):
    ...
    out = path + ".gb"; RUNS += 1
    if os.path.exists(out): os.remove(out)   # F1 (AUDIT4): never classify a stale output
    try:
        r = subprocess.run([MS, "-g", "2", "-t", "1", "-f", path, "-o", out], env=ENV, capture_output=True, text=True, timeout=600)
    except subprocess.TimeoutExpired:
        return None
    if r.returncode != 0 or not os.path.exists(out):   # F1 (AUDIT4): a failed solver run is not a verdict
        return None
...
src = open("$HOME/workspace/claudecode/automath/problems/k1695/round6_controllable.py").read()
G = {}
...
if start_depth > 0:
    ...
    dpll(0, [], "d")
json.dump({"p": p, "chart": chart, "order": [ctype(s) for s in ORDER], "runs": RUNS, "closed": closed, "open": open_nodes, "capped": capped}, open(OUT + "/summary.json", "w"), indent=1)
