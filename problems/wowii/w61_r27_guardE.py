#!/usr/bin/env python3
"""
owner-w61 round 27, task item 3 -- GUARD E: DOES THE BRIEF'S TRANSCRIPTION MATCH
THE SOURCE?
...
ONE PRODUCT: w61_r27_guardE.out.
"""

import re
import sys

BRIEF = "prompts/w61_S3_GFAN_r26.md"
...
def main():
    ...
    rep, findings, rows = run(brief, draft, "SHIPPED BRIEF %s (as sent to Q39)" % BRIEF)
    log.append(rep)
    ...
    out = "\n".join(log)
    open("problems/wowii/w61_r27_guardE.out", "w").write(out + "\n")
    print(out)
    return 0 if (len(findings["NO-MATCH"]) == 0 and allfired) else 1
