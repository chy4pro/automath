...
## YOUR SPECIFIC ANGLE — A: count the ZD correspondence GLOBALLY, never per row
Every previous attempt worked one row (or one point) at a time and died there.  Do the
opposite.  Consider the single incidence set
      I := {(x,c) in M x M : x in D_c}          (|I| = nq, by (F0), two ways)
which by (F11) is ALSO   I = {(x, x*v) : v in Z(x)}   — the SAME set read through the
Z-side.  You therefore have one set with two exact descriptions, one indexed by
columns and one indexed by (point, missed-value) pairs.
Task: extract from this double description a global inequality in which |P| appears
with a coefficient, and push it to |P| <= q.
Concrete sub-tasks, in order:
 1. Write out the exact bijection between the two descriptions of I.  It is a
    bijection {(x,v) : v in Z(x)} -> I, (x,v) |-> (x, x*v).  PROVE injectivity in x's
    fibre (why can two distinct missed values v != v' of the same x not give the same
    column?) — this is the one inversion the angle rests on; if it fails, say so
    loudly and stop, because then the whole angle is dead and that is a valuable
    verdict.
 2. Split I by whether the column is perfect: I = I_P u I_notP with
    I_P = {(x,r) : r in P, x in D_r}, |I_P| = q|P|.  Read I_P through the Z-side:
    I_P = {(x,v) : v in Z(x), x*v in P}.  Hence
        q|P| = Sum_{x in M} #{v in Z(x) : x*v in P}.
    This identity is FREE and unconditional; it is your ground truth.  Now the whole
    problem is: bound  #{v in Z(x) : x*v in P}  above by something summing to < q(q+1).
 3. The pencil is the only extra structure attached to x.  Using (F14) and (F4),
    determine exactly which v in pencil(x) can have x*v in P, carrying the overlap as
    an explicit term (dead route 5 assumed it away — you must NOT).
 4. Under delta == 0, feed in (F2): Sum_v m(v) = nq and m(v) = q b(v).  Ask what
    "every value occurs exactly n times" says about the distribution of the fibres
    #{v in Z(x) : x*v in P} across x, and whether an averaging/convexity step over
    the n points x gives q|P| <= q(q+1) - (something positive).
Deliver: either a PROVED all-q inequality of the form q|P| <= f(q, A, b) with f
evaluated honestly at every A (A is free — see the A-free block), or the single
sharpest identity you can prove about Sum_x #{v in Z(x) : x*v in P}, plus a one-line
statement of exactly what extra fact would close it.  Do not submit step 2 alone: it
is free and already known.

## HARD WORD CAP: 1000 words.
