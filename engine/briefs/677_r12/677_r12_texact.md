...
## YOUR SPECIFIC ANGLE — D: sum (T-exact) over all v, with A carried as a free symbol
(F13) is n unconditional equations.  Nobody has summed them.  Do it, carefully, and
see what the sum says about |P|.
 1. Sum (F13) over all v in M:
        Sum_v |T_v|  +  Sum_v #{x in ell_v : v in Z(x)}  =  n(q+1) + qA,
    where A := #{v : nu^2(v) in ell_v} is FREE design data taking every value 0..n
    (A-free block).  WARNING: a previous brief claimed Sum_v |T_v| = n(q+1)
    unconditionally and was refuted by data; the correct statement is the displayed
    identity with BOTH correction terms present.  Do not drop either.
 2. Identify the second sum combinatorially:
        Sum_v #{x in ell_v : v in Z(x)} = #{(x,v) : x in ell_v, v in Z(x)}
                                        = Sum_x |pencil(x) ^ Z(x)| = Sum_x s_x .
    So   Sum_v |T_v| + Sum_x s_x = n(q+1) + qA   — an exact all-q identity linking a
    row-side quantity, a point-side quantity and the design parameter A.  Prove it
    cleanly; if it is right it is worth registering on its own.
 3. Now bring in perfect rows.  For r in P you know E(r,.) = N(r,.) exactly.  Decide
    precisely, with proof, which v have r in T_v when r is perfect — i.e. compute or
    bound #{v : r in T_v} for a perfect row r, and compare it with the same count for
    an arbitrary row.  If perfect rows are systematically heavier (or lighter) in the
    T-incidence, then Sum_v |T_v| = Sum_t #{v : t in T_v} gives a bound on |P|
    directly.  This is the crux; spend most of your words here.
 4. Under delta == 0 substitute m(v) = q b(v) and Sum_v b(v) = n to eliminate what you
    can.  Report the final inequality as |P| <= <expression in q, A, b>, then evaluate
    it at the extremes A = 0 and A = n and say honestly whether either extreme already
    exceeds q.  An inequality valid only at A = n is worthless here.
Forbidden: treating R_{nu(v)} as a bijection (that is exactly what (F5) exists to
correct — dead routes 2, 3); assuming nu^2(v) in ell_v for any v; assuming b == 1.

## HARD WORD CAP: 900 words.
