...
## YOUR SPECIFIC ANGLE — C: contradiction from a hypothetical (q+1)-set's D-PROFILE
Assume for contradiction that delta == 0 and |P| >= q+1, and FIX q+1 perfect rows
r_0, ..., r_q.  Study the (q+1) x n incidence profile of their missing sets
      D_{r_0}, ..., D_{r_q}   (each of size exactly q),
i.e. the multiset of values and how the D's overlap, and derive a contradiction that
FAILS at |P| = q.  That last requirement is the whole point: any argument that also
kills |P| = q is WRONG, because |P| = q is attained by real objects.  State explicitly,
for every step, why it needs q+1 rather than q.
Structure to work through:
 1. Total incidences: Sum_{i} |D_{r_i}| = q(q+1).  Each value x lies in exactly q of
    the n columns' D-sets (F0), so x can lie in at most q of the q+1 chosen ones; let
    e(x) := #{i : x in D_{r_i}} <= q.  Then Sum_x e(x) = q(q+1) and #{x : e(x) >= 1}
    >= q+1.  NOTE: identities that use only |D_c| = q hold for every (q+1)-set of
    columns, perfect or NOT (dead route 6(ii)) — such a chain is zero-bite.  You must
    bring in PERFECTNESS.  Do it via (F11)+(F14): x in D_{r_i} <=> r_i in C_x <=>
    x\r_i in Z(x).  So the profile is entirely a statement about Z.
 2. Perfectness enters through the array: r perfect means E(r,.) = N(r,.), so row r of
    the Xi-array misses exactly D_r and hits nu(r) exactly q+1 times.  Combine with
    (F4): within ell_{nu(r)} the value nu(r) is hit at most ONCE by row r.  So the
    q+1 hits of nu(r) in row r are spread over points OFF the line, except at most
    one — quantify this, for all q+1 chosen rows at once, and see what it forces on
    the values in the union of the D_{r_i}.
 3. The pigeonhole to hunt: q+1 perfect rows demand q(q+1) "missing" incidences drawn
    from a pool where each value supplies at most q, while the same rows demand
    (q+1)^2 hits of their nu-values, which (F4) confines.  Find the overcount.  Carry
    A, g and b as free symbols (A-free block): a contradiction that needs A = n, or
    b == 1, is void.
 4. If you cannot close it, produce the sharpest PROVED statement of the form
    "|P| >= q+1 forces <explicit configuration>" — an explicit forced configuration is
    a real deliverable, because the next slice can test it directly.  Name the
    configuration in one displayed line.
Forbidden: C_r = D_r or any symmetry of D (dead route 4); "every x in ell_{nu(r)} has
Xi_r(x) = nu(r)" (dead route 3); inverting a column R_c (dead route 2).
