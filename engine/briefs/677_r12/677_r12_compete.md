...
## YOUR SPECIFIC ANGLE — B: the competition statement (ZD-C), with the overlap COUNTED
(F14) says: for each value x, exactly q columns miss x, namely C_x = {x*v : v in Z(x)};
each v in pencil(x) ^ Z(x) FORCES the column nu(v) into C_x, and each
v in pencil(x) \ Z(x) FORBIDS nu(v) from C_x.  Since |pencil(x)| = q+1 > q = |C_x|,
the pencil already competes for every slot.  Perfect rows compete for the same slots:
r in P contributes to C_x exactly when x in D_r.
Set  F_x := {nu(v) : v in pencil(x) ^ Z(x)}  (the pencil-FORCED columns) and
     s_x := |pencil(x) ^ Z(x)|.
Known and free:  F_x subset C_x, so s_x <= q for every x — but ONLY after you check
that v |-> nu(v) is injective on pencil(x) (it is, nu is a bijection — but SAY it).
Task: produce the exact accounting
      |P ^ C_x|  =  |P ^ F_x|  +  |P ^ (C_x \ F_x)|      <=  |P ^ F_x| + (q - s_x)
and then bound Sum_x |P ^ F_x| — the term the previous fan-out ASSUMED to be zero and
was refuted for it (dead route 5; do not repeat).  Two handles, use both:
 * A column r in F_x is nu(v) for some v in pencil(x), i.e. x in ell_v and r = nu(v).
   So r in P ^ F_x forces a very rigid configuration: x lies on the line ell_v whose
   nu-image is a PERFECT row, and v in Z(x).  Use (F12) on that same v: it says
   |ell_v ^ D_{nu(v)}| = #{y in ell_v : v in Z(y)}, so the whole line ell_v is
   involved, not just x.  Convert Sum_x |P ^ F_x| into a sum over PAIRS (v, r) with
   r = nu(v) in P, of |ell_v ^ D_r|.  Get that identity exactly right — it is the
   crux of this angle.
 * Then use (F13)/(F5) to replace |ell_v ^ D_{nu(v)}| by (q+1) + q[nu^2(v) in ell_v]
     - |T_v|, keeping the indicator SYMBOLIC because A is free design data.
Combine with the ground identity  Sum_x |P ^ C_x| = q|P|  (each r in P has |D_r| = q)
to get an inequality on |P|.  Report exactly which term must be controlled to reach
|P| <= q, and whether delta == 0 controls it.
Constraint: at no point may you use "r in D_c <=> c in D_r" (dead route 4), nor treat
R_c as invertible (dead route 2), nor assume nu^2(v) in ell_v (A is free).
Deliver: one PROVED all-q inequality with every term named, plus an honest statement
of the residual gap.  A correct inequality that is too weak beats a strong one with a
silent inversion.
