# ATTACK — Erdős Problem #708, round 4: the hinge inequality (TH) ⇒ g(n) ≤ 18n (single-shot variant, TEMPLATE v2.2)
...
web or any external source. Assume for purposes of this task that (TH) is true and provable; work iteratively: propose,
attack, repair. Partial progress does not count unless it implies exactly one of T1–T5. Check every candidate proof for
hidden use of x = 0, for confusing the thresholds 1 and 2, and for treating per-prime-power domination as sufficient.
Return a complete proof if one survives audit. If none does, return instead: (1) the strongest rigorously proved
derivation as numbered lemmas — e.g. (TH) for 0/1 weights, or with a larger constant c on the left; (2) the exact
remaining gap as a precise statement; (3) machine-checkable artefacts (the tightest instances you found: m, x, z, both
sides). Do not return an empty answer, a bare statement of failure, or an explanation of why the problem is hard. Do
not answer that the problem is open.
