...
## YOUR SPECIFIC ANGLE
Angle 2 -- double counting the quantity  S := Sum_x |pencil(x) ^ Z(x)|.
By (F12), S = Sum_v |ell_v ^ D_{nu(v)}|, and by (F5)/(F13),
Sum_v |T_v| = n(q+1) + qA - S.  Both |pencil(x)| = q+1 and |Z(x)| = q, so each term
is at most q.  Task: bound S from above and below on the delta == 0 sub-branch, using
(F2) (m(v) = q b(v)) and (F14); then bound Sum_v |T_v| both ways in the presence of
|P| = k perfect rows (use (F4): |T_v| >= 1 always, and a perfect row's block-hit count
h_r = #{v : r in T_v} = #{x : Xi_r(x) in pencil(x)}), and collide.  Deliver explicit
constants in q and check non-vacuity at q = 2, n = 7 (there A = n and Tbase = q+1+q
occur, with Sum_v |T_v| observed in 23..35).
