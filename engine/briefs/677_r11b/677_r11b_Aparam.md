...
## YOUR SPECIFIC ANGLE
Angle 6 -- the invariant A = #{c : g(c) = nu^{-1}(c)} = #{v : nu^2(v) in ell_v}.
A is both (F5)'s additive term and the count of columns where g agrees with nu^{-1}.
Task: (1) bound A on the delta == 0 sub-branch from b(v) = |g^{-1}(v)| and
Sum_v b(v) = n; (2) bound A above/below in the presence of k perfect rows via (F7)
and (F13); (3) collide.  Settle whether A = 0 and A = n can occur (at q = 2 a real
realizable design with A = n = 7 and b == 1 exists, so A = n is possible -- explain
what forces it).  Aim at a proved inequality alpha|P| <= A + beta or A <= gamma - |P|
with explicit alpha, beta, gamma in q.
