...
## YOUR SPECIFIC ANGLE
Angle 3 -- the pencil determines part of C_x; make it fight the perfect rows.
By (F14), F_x := {nu(v) : v in pencil(x) ^ Z(x)} is contained in C_x and
G_x := {nu(v) : v in pencil(x) \ Z(x)} is disjoint from C_x, with |F_x| + |G_x| = q+1
and |C_x| = q.  For a perfect row r, membership r in C_x is array-visible.  Task:
(1) prove sharp bounds on |F_x| (can it equal q?  0?), (2) show how many perfect rows
can simultaneously avoid the constraint F_x u {r in P : x in D_r} <= q for all x,
(3) turn (2) into a bound on |P|.  Watch the degenerate case where a perfect row r
lies in G_x for many x.  Prove everything at general q.
