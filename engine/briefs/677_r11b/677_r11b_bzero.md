...
## YOUR SPECIFIC ANGLE
Angle 4 -- the dichotomy b == 1  vs  some b(v) = 0.
Under delta == 0 exactly one holds (see DEAD ROUTES note).  Branch (ii): b(v0) = 0
gives m(v0) = 0, i.e. NO x has x in D_{x*v0}; by (F11) that says v0 is in NO Z(x),
i.e. value v0 occurs in every column of the array.  Extract everything: consequences
for |T_{v0}|, for the D-sets, for the number of perfect rows.  Branch (i): b == 1
means g is a bijection and m == q, i.e. every value is missed by exactly q columns of
the ARRAY as well as of the table.  Decide which branch a hypothetical |P| = q+1
delta==0 object must be in -- proving it must be in one of them and killing that one
is a complete proof.  Prove at general q.
