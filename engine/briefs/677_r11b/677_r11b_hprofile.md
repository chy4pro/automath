...
## YOUR SPECIFIC ANGLE
Angle 5 -- the block-hit profile of a perfect row, now with (ZD).
h_t := #{x : x in ell_{Xi_t(x)}} = #{v : t in T_v} = #{v : t*nu(v) in ell_v}, and
x in ell_{Xi_t(x)} <=> Xi_t(x) = nu^{-1}(Lam_t(x)).  By (F7), for t in ell_c the entry
Xi_t(nu(c)) = g(c) is a block hit exactly when g(c) = nu^{-1}(c), i.e. on the set of
(F6) of size A.  Task: pin or bound h_r for a PERFECT row r at general q (use
perfectness: Xi_r hits nu(r) exactly q+1 times, omits exactly D_r, injective
elsewhere; plus (F4)'s corollary), then sum over P and collide with
Sum_t h_t = n(q+1) + qA - S of Angle 2.  Give exact constants; check at q = 2.
