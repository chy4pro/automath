...
## YOUR SPECIFIC ANGLE
Angle 7 -- the overdetermined system, now unconditional.
(F13) gives n exact equations (one per v) instead of (F5)'s |P|.  Write the whole
system at general q: unknowns = the incidence pattern between the lines ell_v and the
sets Z(x) (equivalently the D-sets, via (F11)), plus the block-hit counts |T_v|.  Add
the delta == 0 identities (every value occurs n times in the array), the perfect-row
profiles, and (F14).  Count equations against degrees of freedom and exhibit a linear
functional that is forced to two different values when |P| = q+1 but not when
|P| = q.  Give the functional explicitly and prove both evaluations.  Instantiate at
q = 2, n = 7 with exact numbers.
