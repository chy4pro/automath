...
## YOUR SPECIFIC ANGLE
Angle 8 -- reverse-engineer the q = 2 kill and generalise it.
FACT (measured, q = 2, one realizable design): with (F4)+(F5)+(F9) alone, 18 arrays
with |P| = 3 = q+1 survive; adding (F11)-(F14) makes EVERY 3-subset infeasible, so
some consequence of (ZD) is doing all the work.  Task: isolate the SMALLEST provable
consequence of (F11)-(F14) that can force |P| <= q, by reasoning (not by search):
propose a short list of candidate "minimal killers", each stated as a lemma with a
complete proof at general q, and for each say precisely which q+1-perfect
configuration it excludes and why the same argument fails for |P| = q.  Prefer
statements that mention BOTH a perfect row's missing set D_r and a vertical missing
set Z(x), since only those two are coupled by (ZD).
