...
## YOUR SPECIFIC ANGLE
Angle C -- amplifying the closure (F9) with delta == 0.
The closure is sound but is evaded by arrays with FEW block hits; (F4) forces block
hits.  Your task: find NEW SOUND closure rules that use delta == 0.  Candidates to
develop (prove or refute each): (1) from delta(v) = 0 and (F1), the number
m(v) = q b(v) is known before the table is known -- turn that into a counting rule on
the unknown cells of column v; (2) for v with nu(v) perfect, |T_v| is pinned by the
array AND by (F4); equate the two and read off |ell_v ^ D_{nu(v)}| exactly, then feed
that back as "exactly this many of the q+1 cells of ell_v are omitted values of column
nu(v)"; (3) propagate omitted-value information (Z-matrix) through the forward/backward
rules.  Deliver rules with proofs of soundness and a statement of what each one adds
that (F3),(F4),(F9) alone do not.
