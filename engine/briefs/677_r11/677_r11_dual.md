...
## YOUR SPECIFIC ANGLE
Angle D -- exploit the duality asymmetry (F10).
D(a,b) = a\b is a branch object with nu_D = nu^{-1}, ell^D_b = ell_{nu^{-1}(b)},
N_D = N^T; but delta == 0 is NOT preserved.  Compute EXACTLY how delta transforms under
the duality: express delta_D(v) in terms of the primal data (b, m, nu, the D-sets).
Then: perfect ROWS of the primal correspond to what in the dual?  If delta == 0 fails
in the dual, the full-branch bound |P| <= q+1 still applies there -- so a primal object
with q+1 perfect rows and delta == 0 maps to a dual object with a computable perfect
count and a computable delta.  Try to close the one-row gap by playing the primal
delta == 0 against the dual's own constraint set.  Prove the transformation law first;
it is the deliverable even if the kill does not follow.
