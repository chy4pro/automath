...
## YOUR SPECIFIC ANGLE
Angle B0 -- the b(v) = 0 branch.
Develop case (ii) above to the maximum.  Suppose delta == 0 and b(v0) = 0 for some
v0.  Then m(v0) = 0, i.e. for EVERY x, x is not omitted by column x*v0.  Extract every
consequence you can: what does it force about the sets D_c (a system of q-subsets with
Sum_c |D_c| = nq and each value omitted by exactly q columns), about R_{v0}, about
|T_{v0}| and about block hits h_t; and then how many perfect rows can coexist with such
a v0.  Then attack the complementary question: given delta == 0 and |P| = q+1, can you
PROVE that some b(v) = 0 (equivalently that g is not a bijection), or PROVE the
opposite?  A proved dichotomy-and-kill of one side is the deliverable.
