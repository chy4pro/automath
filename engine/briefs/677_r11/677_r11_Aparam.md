...
## YOUR SPECIFIC ANGLE
Angle A -- the invariant A = #{c : g(c) = nu^{-1}(c)} = #{v : nu^2(v) in ell_v}.
A is simultaneously (F4)'s additive term and a measure of how far g is from nu^{-1}.
Task: (1) bound A on the delta == 0 sub-branch, using b(v) = |g^{-1}(v)| and
Sum_v b(v) = n; (2) bound A from below/above in the presence of |P| = k perfect rows,
using (F6) and perfectness; (3) collide the two.  Also settle: can A = 0 happen?  can
A = n?  what is A on a table with delta == 0?  Every claim needs a proof or an explicit
consistent instance.  Aim for a proved inequality of the shape
 alpha*|P| <= A + beta  or  A <= gamma - |P| , with explicit alpha, beta, gamma in q.
