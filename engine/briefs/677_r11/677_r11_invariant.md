...
## YOUR SPECIFIC ANGLE
Angle I -- design a SEPARATING INVARIANT.
Fact: on one realizable q=2 design there exist exactly 18 arrays with |P| = 3 = q+1
that satisfy the plane structure, the forced skeleton (F6), the exact column structure,
delta == 0, (F3), (F4) and the (F9) closure, yet NONE of them is realized by a table
(all 3888 real tables of that design have |P| <= 2).  So some PROVED property of real
objects is still missing from that list.  Your task: propose candidate proved
properties of genuine branch objects -- necessarily cross-row, necessarily using
realizability (F2) -- that are NOT consequences of the listed system, each with a full
proof from (a)-(c) and (F0)-(F10), and each stated so that it is CHECKABLE on an
Xi-array plus design data alone.  Prioritise properties that involve delta == 0 and the
D-sets simultaneously.  Rank your candidates by how likely they are to fail on an array
with q+1 perfect rows, and say for each exactly what to compute to test it.
