...
## YOUR SPECIFIC ANGLE
Angle S -- the pinned linear system on nu^{-1}(P).
For each v with nu(v) in P, (F4) pins |T_v| exactly from the array.  With |P| = q+1
that is q+1 simultaneous exact equations coupling the array to the design.  Write the
system out in full at general q: unknowns = the incidence pattern between the lines
ell_v and the omitted sets D_{nu(v)}, plus the block-hit counts.  Add the delta == 0
identities Sum_t E(t,v) = n and the perfect-row profiles.  Look for an overdetermination:
count equations against degrees of freedom, and find the linear functional that is
forced to two different values when |P| = q+1 but not when |P| = q.  Give the functional
explicitly and prove the two evaluations.  Instantiate at q = 2 (n = 7) with exact
numbers to show the count is not vacuous.
