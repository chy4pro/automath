...
## YOUR SPECIFIC ANGLE
Angle M1 -- the b == 1 branch (m == q equidistribution).
Assume delta == 0 with b == 1, i.e. g : M -> M is a bijection and m(v) = q for EVERY v.
Introduce the 0/1 matrix Z(c,x) := [x in D_c]; it is q-regular in BOTH indices (F0).
Then m(v) = Sum_x Z(x*v, x), and delta == 0 says this "twisted trace" of Z along the
non-injective map R_v equals q for every v, simultaneously.  Split it as
 m(v) = |ell_v ^ D_{nu(v)}| + #{c not in D_v u {nu(v)} : R_v^{-1}(c) in D_c}
and mine the resulting exact system, especially in combination with (F4)
(|T_v| = (q+1) + q[nu^2(v) in ell_v] - |ell_v ^ D_{nu(v)}|) and with perfect rows
(where D is array-visible).  Target: an inequality on |P| that is strictly stronger
than |P| <= q+1.
