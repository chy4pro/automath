# ATTACK — Erdős Problem #1212, round 4: the turn-weighted Peierls estimate (single-shot variant, TEMPLATE v2.2)
...
proved derivation you reached, as numbered lemmas — e.g. the turn-charging bound A(k) with its constant, the covering
lower bound ∏p ≥ exp(c√m) for closed straight runs, exact worst-case δ for contours of length ≤ 14; (2) the exact
remaining gap as a precise open statement; (3) every machine-checkable artefact you built (tables, extremal contours,
code). Do not return an empty answer, a bare statement of failure, a status report, or an explanation of why the
