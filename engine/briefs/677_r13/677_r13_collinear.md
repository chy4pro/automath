...
## YOUR SPECIFIC ANGLE — B: the COLLINEAR case, which the arc form leaves open
(F15) has an arc form for q+2 perfect rows with NO 3 collinear.  The gap it leaves is the
COLLINEAR configuration, and an earlier attempt to kill collinear 4-sets failed for a
reason you must not repeat: it derived its contradiction from the POSITIONAL clause of
(F7), which is automatic and vacuous (see (F7)).  The only content available at a joining
column is the NON-MISSED clause of (F15).
Setting for you: suppose L is a line of the plane and P ^ L contains at least 3 points
(at least 3 collinear perfect rows).  Let c0 be the column with ell_{c0} = L (unique, by
(b)).  Then c0 is in pencil(r) for EVERY r in L, so by (F15)(i) the single value g(c0) is
missing from NO D_r for r in P ^ L, i.e. g(c0) not in Union_{r in P^L} D_r.
Sub-tasks, in order:
 1. State exactly what q+1 constraints the single column c0 imposes on the rows of L,
    using (F15) and (F7) — and mark which of them are vacuous by (F7).  Do not use the
    positional clause.
 2. Turn "g(c0) not in D_r for all r in P ^ L" into a statement about C_{g(c0)}, whose
    size is exactly q by (F0): P ^ L ^ C_{g(c0)} = empty.  Since |C_{g(c0)}| = q and
    |L| = q+1, count how many points of L are already spent, and what that leaves for
    |P ^ L|.  Be exact about whether C_{g(c0)} is a subset of L (it need not be — say so
    with the bracketed reason).
 3. Now vary the line: every point r of the plane lies on q+1 lines, and every pair of
    perfect rows is joined by exactly one column.  Count the pairs of perfect rows two
    ways: Sum over columns c of C(|P ^ ell_c|, 2) = C(|P|, 2).  This is a plane-geometry
    identity, NOT the retired I-double-count — confirm that in your opening line.  Feed
    in the per-line bound from step 2 and see what cap on |P| it yields.
 4. If step 3 yields only |P| <= q+1 or weaker, find the ONE extra ingredient that
    separates |P| = q+1 from |P| = q, and prove it or name it precisely as a CONJECTURE.
    (F15)(ii) with b > 1 is the intended candidate: a repeated g-value inside pencil(r)
    forces that value to be nu(r).
Deliver: a PROVED bound on |P ^ ell_c| for a single line, at all q, with every case of
the configuration of C_{g(c)} treated; then the best global cap that bound implies via
the pair count of step 3.
