...
## YOUR SPECIFIC ANGLE — A: the g-fibre LINE UNION exclusion (this is where (F15)(i) bites)
(F15)(i) says: r perfect and c in pencil(r) ==> g(c) not in D_r.  Rewrite it with the
C-notation of (F14): x in D_r <=> r in C_x, so (F15)(i) is exactly
      **for every value x:   P ^ C_x ^ U_x = empty,  where  U_x := union of ell_c over
      the g-fibre  c in g^{-1}(x).**
Read that carefully: it is a GEOMETRIC exclusion between the q columns of C_x and a union
of b(x) LINES of the plane.  This is the term the retired global double count does not
have — say so in your opening line, and keep the ground identity q|P| = Sum_x |P ^ C_x|
only as bookkeeping.
Sub-tasks, in order:
 1. Prove the displayed reformulation of (F15)(i) from (F14) + (F15)(i), with the
    bracketed argument of every step (V3).  Note b(x) = 0 makes U_x empty and the
    exclusion vacuous at that x — state that explicitly (V2).
 2. Compute |U_x| EXACTLY as a function of the configuration of the b(x) lines
    {ell_c : g(c) = x}, using only the plane axioms: any two distinct lines meet in
    exactly one point.  Do the cases b(x) = 0, 1, 2, 3 in full (for b(x) = 3 both the
    concurrent and the triangle configuration), then give the general formula in terms of
    the number of points of multiplicity >= 2 in the multiset union.  DO NOT sum
    |ell_c| over the fibre — that is dead route 8(ii).
 3. Under delta == 0 you also have (F8) b(x) <= q and (F0) Sum_x b(x) = n.  Combine with
    step 2 to lower-bound Sum_x |U_x|, honestly, with the multiplicity defect carried as
    an explicit symbol rather than dropped.
 4. Now push: each x contributes |P ^ C_x| <= min(q, |P|, ...) and its perfect members
    must avoid U_x.  Turn the n exclusions of step 1 into ONE inequality in |P|.  The
    natural handle is: sum over x of |P ^ C_x| = q|P| while every summand lives in
    M \ U_x.  Ask what forces the P-points into the complement of many U_x at once —
    a point r lies outside U_x for every x it "sees" via C, and r itself carries the
    q+1 columns of pencil(r), whose g-values are constrained by (F15)(ii).
Deliver: either a PROVED all-q inequality in |P| with A and the multiplicity defect
carried symbolically, or the exact form of Sum_x |P ^ C_x ^ U_x| = 0 as a constraint on
the pair (g, D) together with the single sharpest consequence you can prove, plus one
line naming exactly what extra fact would close it to |P| <= q.
