...
## YOUR SPECIFIC ANGLE — C: run the phi-fusion closure SYMBOLICALLY at general q
The named target says "(F11)-(F15) + the phi-fusion closure (F9)".  (F9) is the only
non-counting ingredient available, and every counting-only route above is now retired, so
this angle is the load-bearing one.  A previous attempt at it died on ONE mechanical
fault: it transported along phi with the row index in the point slot (dead route 7).  You
must state the ARGUMENT of every transport (V3) — that alone is what makes this angle
survivable.
Setting: assume delta == 0 and assume q+1 perfect rows r_0, ..., r_q.  Derive a
contradiction with (F9).
Sub-tasks, in order:
 1. Write the closure rules of (F9) as a rewriting system on PARTIAL table cells, with
    the argument slots explicit: for a known cell x*v = u, the forward rule produces
    t*u = x where v = Xi_t(x); for a known Lam_t(x) = c, the backward rule produces
    x*v = c.  For each rule, state which map is applied to which element in which slot.
 2. Seed with the block cells: a*c = nu(c) for every a in ell_c.  That is n(q+1) cells,
    known with no hypothesis.  Propagate ONE step and write down exactly which new cells
    appear, symbolically in (nu, g, ell).  Use (F7) to identify the array cells you get
    for free; use (F11) to identify which cells are FORBIDDEN (the D-sets).
 3. Now add ONE perfect row r: perfectness says column r of the TABLE carries exactly
    M \ ({nu(r)} u D_r) off the block, and row r of the ARRAY has fibre profile equal to
    N(r,.).  Propagate again and record what perfectness of a single row forces about the
    g-values of pencil(r) — you should recover (F15)(i)(ii); if you do not, your rules
    are wrong, so use that as a self-check and SAY whether it passed.
 4. Add a second perfect row s and the joining column c0.  Propagate to a fixpoint or to
    a conflict.  The quantity to watch is the pair (g(c0), nu(c0)) and the two constraints
    of (F15) at the joining column.  Then attempt q+1 perfect rows.
 5. If you reach a conflict, state the MINIMAL hypothesis set it used (which of delta==0,
    (F8), b-values, A-value) and whether it is general-q or only q = 2.  If you do not,
    deliver the exact fixpoint you reached — the set of forced cells as a function of the
    design data — since that is itself a reusable object.
Deliver: the closure rules with explicit argument slots, the one-step and two-step
fixpoints in symbolic form, the self-check of step 3 reported honestly, and either a
conflict at q+1 perfect rows or the precise obstruction that stops the propagation.
