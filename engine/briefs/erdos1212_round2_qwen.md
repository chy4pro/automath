# ATTACK — Erdős Problem #1212, strengthened infinite path — ROUND 2 (single-shot variant, TEMPLATE v2.2)
...
rigorously proved derivation you reached, as numbered lemmas — for instance the Peierls estimate R-A proved for
contours up to a stated length, or R-B proved under an explicitly stated extra hypothesis, or a proved lemma bounding
how many vertices of a contour a single prime can close; (2) the exact remaining gap as a precise open statement;
(3) every machine-checkable artefact you built (explicit rails, residues, contour counts, path prefixes) so it can be
re-run and reused. Do not return an empty answer, a bare statement of failure, a status report, or an explanation of
why the problem is hard. Do not search the web to determine whether the statement is open, and do not answer that it
is open.
