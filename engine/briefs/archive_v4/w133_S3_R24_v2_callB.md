...
---
DELIVERY OVERRIDE (CALL B of a two-call split; you already delivered — in CALL A — the
execution declaration, the 8-row held-out table, and joints R12-M1..M4; do NOT repeat
them):
Deliver ONLY, hard cap 1100 words total:
1. Joints R12-B1..R12-B3 verdict rows.
2. The two mandatory probes + the configuration-vs-graph currency probe.
3. Defect log (each defect: statement, why, MATHEMATICS|BOOKKEEPING tag).
4. Line 1 of this delivery must be: MATHEMATICS DEFECT FOUND: YES|NO — judged over your
   ENTIRE review (both calls).
End with: END-OF-CALL-B