...
5 internal status annotations that have been removed** — governance
...
**"verbatim except 5 declared annotations"**, which is weaker than
...
| **V10** | **H** | K1: the maximum degree, and the vertex attaining it (the answer is unique) |
| **V11** | **H** | K2: the trace census (/W_1/, /W_cons/, /W_anti/, /W_0/) |
| **V12** | **C** | K2: T_Z, and sum_{z in Z}(a(z)-3) |
...
**So the question `a(v) ≤ ?` for `|N(v) ∩ Z| ≤ 1` has BOTH answers inside it:** the
