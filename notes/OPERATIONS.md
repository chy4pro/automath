...
**Current state (2026-09-25): continuation mechanism = `CronCreate` inside the Fable coordinator session, TESTED.** A one-shot job scheduled 11:28Z fired at 11:33Z (ledger entry "TICK-TEST fired"). Properties: session-only (dies with the session, not on disk), fires only while the REPL is idle, recurring jobs auto-expire after 7 days and must be re-armed; the coordinator records each armed job here. Lean policy (owner 2026-09-25): Lean only for a result worth verifying; the #708 FinalCheck container rebuild is dropped from the queue. Production tick ARMED 2026-09-25 ~12:50Z: CronCreate job d1edc358, hourly at :07, 7-day expiry (re-arm by 2026-10-02); prompt = read notifications, check in-flight agents, act per gates, one ledger line if changed. Task notifications (subagents, workflows, background commands) are the primary wake signal; the tick is the fallback.
