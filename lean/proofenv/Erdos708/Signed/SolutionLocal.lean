...
theorem sparse_core (A : AtomSystem) (m : ℕ) (hm : 4096 < m) (hH : mean A < 17 / 16) (x : ℕ) :
  ...
  obtain ⟨hmean, hright, hleft⟩ := rounding A m hm x
  have hfeasible : (∑ n ∈ Icc (x + 1) (x + m), F A m n) ≤ RB A m x := by
    apply sum_le_sum
    intro n hn
    exact pointwise_feasibility A m hm hH n (by have := (mem_Icc.mp hn).1; omega)
  have hvalue := (value_on_window A m hm hH x).2.2.2
  linarith
...
theorem solution (A : Erdos708SparseCore.AtomSystem) (m : ℕ) (hm : 4096 < m) (hH : Erdos708SparseCore.mean A < 17 / 16) (x : ℕ) :
    (141 / 128 : ℝ) * Erdos708SparseCore.L A m ≤ Erdos708SparseCore.R A x m :=
  Erdos708SparseCore.sparse_core A m hm hH x
