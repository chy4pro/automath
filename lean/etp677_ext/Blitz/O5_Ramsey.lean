...
As of Mathlib at the time of writing, Ramsey's theorem is *not* in Mathlib (the word "Ramsey"
occurs only in prose inside `Mathlib/Combinatorics/Hindman.lean`,
`Mathlib/Combinatorics/HalesJewett.lean` and `Archive/Wiedijk100Theorems/
AscendingDescendingSequences.lean`), so this is a genuinely new formalisation.
