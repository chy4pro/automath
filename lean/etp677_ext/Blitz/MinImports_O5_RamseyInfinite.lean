...
As of Mathlib at the time of writing, Ramsey's theorem — finite or infinite — is not in Mathlib.
...
theorem chain_subset_of_le (c : ℕ → ℕ → β) (f : ℕ → β) (B : Set ℕ) {i j : ℕ} (h : i ≤ j) :
